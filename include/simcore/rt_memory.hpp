...
class FrameArena {
public:
    explicit FrameArena(std::size_t capacityBytes = (1u << 20), std::size_t alignment = 64)
    ...
    FrameArena(FrameArena&& o) noexcept
        : buffer_(o.buffer_), capacity_(o.capacity_), head_(o.head_), alignment_(o.alignment_)
#if defined(__linux__)
        , pageSize_(o.pageSize_)
#endif
    {
        o.buffer_ = nullptr; o.capacity_ = 0; o.head_ = 0;
    }

    FrameArena& operator=(FrameArena&& o) noexcept {
        if (this != &o) {
            release_();
            buffer_   = o.buffer_;   o.buffer_   = nullptr;
            capacity_ = o.capacity_; o.capacity_ = 0;
            head_     = o.head_;     o.head_     = 0;
    ...
    struct Marker { std::size_t head; };
    Marker mark() const noexcept { return Marker{ head_ }; }
    void   rewind(Marker m) noexcept { assert(m.head <= head_); head_ = m.head; }
...
private:
    void reserve_(std::size_t cap) {
        ...
        capacity_ = cap;
        if (capacity_) {
            buffer_ = static_cast<std::byte*>(::operator new(capacity_, std::align_val_t(alignment_)));
#if defined(__linux__)
    ...
    void release_() noexcept {
        if (buffer_) {
            ...
            buffer_ = nullptr; capacity_ = 0; head_ = 0;
        }
    }
    ...
    void pre_touch_() noexcept {
        const std::size_t page =
            ...
            os_page_size();
#endif
        for (std::size_t i = 0; i < capacity_; i += page) buffer_[i] = std::byte{0};
        if (capacity_) buffer_[capacity_ - 1] = std::byte{0};
    }
...
public:
    FrameArenaPool(std::size_t threads,
        ...
        : arenas_(), nextIndex_(0)
    {
        arenas_.reserve(threads);
        for (std::size_t i = 0; i < threads; ++i)
            arenas_.emplace_back(perThreadCapacityBytes, baseAlignment);
    ...
    FrameArena& tls() noexcept {
        assert(tlsIndex_ != static_cast<std::size_t>(-1) && "Thread not bound; call bindCurrentThread(i)");
...
inline thread_local std::size_t FrameArenaPool::tlsIndex_ = static_cast<std::size_t>(-1);
