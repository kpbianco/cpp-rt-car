...
inline HazardRecord* acquire_hazard() {
    ...
    do {
        rec->next = old;
    } while (!global_hazard_head.compare_exchange_weak(old, rec, std::memory_order_release, std::memory_order_relaxed));
    return rec;
}
...
inline void scan() {
    ...
    for (HazardRecord* r = global_hazard_head.load(std::memory_order_acquire); r; r = r->next) {
    ...
    while (it != list.end()) {
        ...
        for (void* h : hazards) {
            if (h == p) { protected_ptr = true; break; }
        }
        if (!protected_ptr) {
            it->deleter(p);
            it = list.erase(it);
