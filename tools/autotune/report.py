...
if __package__ is None or __package__ == "":
    ...
    from tools.autotune import common_host  # type: ignore
else:  # pragma: no cover - import path when executed as module
    from . import common_host  # type: ignore
...
def _detect_spec_digest(spec_path: pathlib.Path, summary_path: pathlib.Path) -> str:
    if summary_path.is_file():
        try:
            ...
            digest = summary_payload.get("spec_digest")
...
def main() -> None:
    ...
    spec_digest = _detect_spec_digest(args.spec, results_dir / "summary.json")
    python_version = _platform.python_version()
    os_name = common_host.detect_os_name()
    cpu_slug = common_host.slugify_cpu(common_host.detect_cpu_model())
    ...
    lines.append(f"Spec digest: {spec_digest}")
    lines.append(
        "Generated by: commit "
        f"{git_commit} / Python {python_version} / {os_name} / {cpu_slug}"
    )
