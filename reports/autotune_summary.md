...
Archived pre-M13 tooling fixture; it was not generated by the current
`tools/autotune/spec.yaml`.

> **Illustrative fixture only.** The tracked data is not a valid measurement
> of `rtfw_runtime_demo` and its retired factors do not map to the supported
> runtime profile schema. Do not deploy this profile or cite these values as
> runtime results.
...
```bash
# Current profiles use the separate target-runtime demo.
./build/rtfw_runtime_demo --config profiles/<cpu>-<os>.json --rt --run 5s --metrics-json-interval
```
