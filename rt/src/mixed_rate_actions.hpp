...
static_assert(
    ...
    "mixed-rate actions require lock-free 64-bit atomics");
...
class MixedRateActionRing final {
public:
    explicit MixedRateActionRing(std::size_t capacity);
    ~MixedRateActionRing();

    MixedRateActionRing(const MixedRateActionRing&) = delete;
    MixedRateActionRing& operator=(const MixedRateActionRing&) = delete;
...
private:
    ...
    static_assert(sizeof(MixedRateActionRecord) % sizeof(std::uint64_t) == 0);
...
constexpr std::size_t MixedRateActionRing::slot_size() noexcept {
    return sizeof(Slot);
}

constexpr std::size_t MixedRateActionRing::slot_alignment() noexcept {
    return alignof(Slot);
}

[[nodiscard]] bool mixed_rate_action_valid(
