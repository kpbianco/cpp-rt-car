...
public:
    ...
    bool pop(int& out) {
        ...
        while (h) {
            guard.protect(reinterpret_cast<std::atomic<void*>&>(head));
...
TEST(HazardPointer, StackCorrectness) {
    LockFreeStack st;
    const int N = 1000;
    std::thread t1([&]{ for (int i=0;i<N;++i) st.push(i); });
    std::thread t2([&]{ for (int i=0;i<N;++i) st.push(i); });
    t1.join(); t2.join();
    std::vector<int> vals; vals.reserve(2*N);
    std::thread c1([&]{ int x; while (st.pop(x)) vals.push_back(x); });
    std::thread c2([&]{ int x; while (st.pop(x)) vals.push_back(x); });
    c1.join(); c2.join();
