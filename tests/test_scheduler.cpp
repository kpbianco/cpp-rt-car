...
TEST(Scheduler, EDFvsFixedFrame) {
  ...
  // EDF policy
  {
    ...
    sched.addTask(rt::Task{[&] { order.push_back('A'); },
                           std::chrono::steady_clock::now() + 10ms, 5ms, 0});
    sched.addTask(rt::Task{[&] { order.push_back('B'); },
                           std::chrono::steady_clock::now() + 5ms, 20ms, 0});
...
TEST(Scheduler, AgingPreventsStarvation) {
  std::deque<rt::Task> q;
  // Low priority task
  q.push_back(rt::Task{[] {}, std::chrono::steady_clock::now(), 1ms, 0});
  // High priority task that we will repeatedly reinsert
  q.push_back(rt::Task{[] {}, std::chrono::steady_clock::now(), 1ms, 5});

  bool low_seen = false;
  for (int i = 0; i < 10; ++i) {
    rt::Task t = rt::pop_next_with_aging(q);
    if (t.priority == 0) {
      low_seen = true;
      break;
    }
