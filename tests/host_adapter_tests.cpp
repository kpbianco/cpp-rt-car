...
rt::TaskResult count_range(
    ...
    require(context.scratch().size() == 64);
    require(context.scratch().data() != nullptr);
...
rt::CallbackResult run_root(
    ...
    require(task_scratch.data() == workload.host->active_scratch);
    require(task_scratch.size() == workload.host->active_scratch_bytes);
    return context.tasks.parallel_for(
               32,
               4,
               &count_range,
...
void cpp_host_adapter_contract() {
    ...
    require(missing.configure(missing_config) == rt::Status::ok);
    require(missing.finalize() == rt::Status::invalid_config);
    ...
    require(runtime.set_host_executor(adapter) == rt::Status::ok);

    Workload workload{&host};
    rt::PhaseHandle root;
    rt::PhaseHandle dependent;
    require(runtime.register_callback(
               {"root", &run_root, &workload},
               root) == rt::Status::ok);
    require(runtime.register_callback(
               {"dependent", &run_dependent, &workload},
               dependent) == rt::Status::ok);
    require(runtime.add_dependency(root, dependent) == rt::Status::ok);
    require(runtime.finalize() == rt::Status::ok);
    rt::MemoryPlan plan;
    require(runtime.memory_plan(plan));
    require(plan.queue_slots == 8);
    ...
    require(first_status == rt::Status::ok);
    require(allocation_count.load(std::memory_order_acquire) == 0);
    require(result.callbacks_executed == 2);
    require(workload.phase_calls == 1);
    require(workload.range_items == 32);
    require(workload.dependent_calls == 1);
    require(host.submitted == 10);
    require(host.executed == 10);

    const auto stats = runtime.executor_stats();
    require(stats.policy == rt::ExecutorPolicy::host_adapter);
    require(stats.worker_count == 2);
    require(stats.worker_starts == 0);
    require(stats.submitted_tasks == 10);
    ...
    host.retained.execute(
        ...
        0);
    require(workload.phase_calls == calls_before_stale);
    ...
    require(overloaded.configure(overload_config) == rt::Status::ok);
    require(overloaded.set_host_executor({
               &saturated,
               1,
               8,
               &FakeHost::submit,
               &FakeHost::try_execute_one}) == rt::Status::ok);
    require(overloaded.register_callback(
               {"root", &run_dependent, &workload}) == rt::Status::ok);
    require(overloaded.finalize() == rt::Status::ok);
    require(overloaded.start() == rt::Status::ok);
    require(overloaded.step(
               {3, std::chrono::nanoseconds(1), std::nullopt}) ==
           rt::Status::queue_full);
    require(overloaded.stop() == rt::Status::ok);
}
...
void threaded_host_adapter_contract() {
    ...
    require(runtime.configure(config) == rt::Status::ok);
    require(runtime.set_host_executor({
               &host,
    ...
    require(allocation_count.load(std::memory_order_acquire) == 0);
    require(workload.roots.load(std::memory_order_acquire) == 1);
    require(workload.range_items.load(std::memory_order_acquire) == 256);
    require(workload.dependents.load(std::memory_order_acquire) == 1);
    require(host.submitted.load(std::memory_order_acquire) == 66);
    for (std::size_t attempt = 0;
    ...
    require(host.executed.load(std::memory_order_acquire) == 66);
    const auto stats = runtime.executor_stats();
    require(stats.worker_starts == 0);
    require(stats.local_executions == 66);
    require(runtime.stop() == rt::Status::ok);
}
...
void c_host_adapter_contract() {
    ...
    require(rtfw_get_abi_info(&info) == RTFW_STATUS_OK);
    require(info.abi_version == RTFW_C_ABI_VERSION);
    require(info.layout_fingerprint ==
           RTFW_C_ABI_LAYOUT_FINGERPRINT);
    require((info.feature_flags &
    ...
    require(rtfw_create(&config, &runtime) == RTFW_STATUS_OK);
    ...
    require(rtfw_set_host_executor(runtime, &adapter) ==
           RTFW_STATUS_OK);
    require(rtfw_register_callback(
               runtime,
               "c.root",
               &c_root,
               &host) == RTFW_STATUS_OK);
    require(rtfw_finalize(runtime) == RTFW_STATUS_OK);
    require(rtfw_start(runtime) == RTFW_STATUS_OK);

    rtfw_frame_context frame;
    rtfw_frame_context_init(&frame);
    frame.frame_index = 1;
    frame.delta_ns = 1'000'000;
    require(rtfw_step(runtime, &frame, nullptr) == RTFW_STATUS_OK);
    require(host.callback_count == 1);
    require(host.range_items == 16);
    require(rtfw_stop(runtime) == RTFW_STATUS_OK);
