...
| Device-rate model and admission | M21-01 implementation candidate | Additive C++ bindings attach completion budgets, in-flight bounds, and access-matched payload roles to copied HAL-v2 command-batch declarations. Finalization builds an inspectable mixed CPU/device reference plan and conservative cyclic admission report; active device dispatch remains rejected before callbacks until M21-02. |
