...
| Active device-rate dispatch | M21-02 implementation candidate | M21-01's copied HAL-v2 command-batch model and cyclic admission now feed precomputed release/dependency indexes and generation-safe completion tickets. Active mixed plans dispatch through existing backend lanes, overlap independent releases, preserve dependency/group barriers, clamp timeouts, and quarantine late completion after timeout. No payload, sampled-I/O, hardware, or RT qualification claim follows. |
