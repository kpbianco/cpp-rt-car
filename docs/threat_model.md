...
| Supply-chain substitution | Exact submodule/FetchContent/vcpkg/action reconciliation, blocking high-severity dependency review, a checked release contract, canonical candidate SPDX SBOMs, expected-source manifests, and unsigned candidate statements exist. A public non-target signed fixture exercises offline verification, but no RTFW candidate is signed or authenticated. |
| Assurance-input drift | M20-PRE-01 rejects source-manifest additions/omissions/duplicates, mutable or unknown dependencies/actions, corpus/dictionary digest drift, malformed/unsafe candidate inventories, wrong expected source, unsigned-statement drift, and public-fixture identity/signature mutations. Hand-selected finite fuzz seeds, a specialized SPDX shape verifier, stale public trust, and declared dependency policy remain incomplete security coverage. |
...
Future hardening needs continuous mutation/fuzz service coverage beyond the
bounded M20-PRE-01 smokes, bounds and resource ceilings for every remaining
untrusted-input parser, backend timeouts/reset, authenticated signed target
release provenance with trust/revocation policy, exporter access policy, and a
documented trusted-plugin policy.
