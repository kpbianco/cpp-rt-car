...
RTFW 0.9 is an in-process native library prototype. The host application,
...
| Device hang/loss | M8 defines timeout/loss statuses, poll-only completion, health/reset/shutdown, and deterministic mock evidence; a malicious backend still shares process authority, and no hardware-driver recovery is qualified |
