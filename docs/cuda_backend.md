...
RTFW 0.10 contains the M9 CUDA Driver API backend candidate. The backend is a
...
There is no qualified tuple in the 0.10 support matrix.
...
self-hosted runner labeled `nvidia-gpu` and uploads the raw JSON. A passing run
is evidence for review, not an automatic support-matrix mutation.
...
No tuple has completed those gates in the repository at 0.10.0.
